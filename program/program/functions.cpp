...
void addNodeToTable(node* currentNode, QHash<int, node*>& nodeTable){
	...
	// Добавить вершину в таблицу
	nodeTable[currentNode->data] = currentNode;

	node* ret = NULL;
	int n = currentNode->children.size();
	// Добавить вершины на поддеревьях в таблицу
	for (int i = 0; i < n; i++){
		addNodeToTable(currentNode->children[i], nodeTable);
	}
}
...
int findRootMaxCommonTree(node* root1, node* root2, node*& retRoot, const QHash<int, node*>& nodeTableTree2){
	...
	node* sameData = nodeTableTree2[root1->data];
